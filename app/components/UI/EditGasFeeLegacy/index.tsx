...
interface EditGasFeeLegacyProps {
  ...
  gasFee: any;
  gasOptions: any;
  onChange: (gas: any, selectedOption: string | null) => void;
  onCancel: () => void;
...
const EditGasFeeLegacy: React.FC<EditGasFeeLegacyProps> = ({
  ...
  const [showRangeInfoModal, setShowRangeInfoModal] = useState<string | boolean>(false);
  const [showAdvancedOptions, setShowAdvancedOptions] = useState(
    !selected || onlyAdvanced,
  );
  const [selectedOption, setSelectedOption] = useState<string | null | undefined>(selected);
  ...
  const changeGas = (gas: any, selectedOption: string | null) => {
  ...
  const renderOptions = () =>
    [
      ...
      .filter(({ name }: any) => !shouldIgnore(name))
      .map(({ name, label, ...option }: any) => ({
        name,
