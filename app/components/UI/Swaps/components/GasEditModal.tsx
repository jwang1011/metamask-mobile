...
interface GasEditModalOwnProps {
  dismiss?: () => void;
  ...
  isVisible?: boolean;
  onGasUpdate: (gasFee: any, gasLimit: string) => void;
  customGasFee?: {
...
type GasEditModalProps = PropsFromRedux & GasEditModalOwnProps;
...
const GasEditModal: React.FC<GasEditModalProps> = ({
  ...
  const [EIP1559TransactionDataTemp, setEIP1559TransactionDataTemp] = useState<any>(
    {},
  );
  const [LegacyTransactionDataTemp, setLegacyTransactionDataTemp] = useState<any>(
    {},
  );
  const [
  ...
  const calculateTempGasFeeLegacy = useCallback(
    ({ suggestedGasLimit, suggestedGasPrice }: any, selected: string | null) => {
...
export default connector(GasEditModal);
